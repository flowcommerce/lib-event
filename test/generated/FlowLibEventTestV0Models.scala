 ...
 * apibuilder:0.12.3 https://api.apibuilder.io/flow/lib-event-test/0.0.1/play_2_x_json
 */
      ...
      def reads(js: play.api.libs.json.JsValue): play.api.libs.json.JsResult[TestEvent] = {
        (js \ "discriminator").validateOpt[String] match {
          ...
          case play.api.libs.json.JsSuccess(discriminator, _) => {
            discriminator match {
              case Some("test_object_upserted") => js.validate[io.flow.lib.event.test.v0.models.TestObjectUpserted]
              case Some("test_object_deleted") => js.validate[io.flow.lib.event.test.v0.models.TestObjectDeleted]
              case Some(other) => play.api.libs.json.JsSuccess(io.flow.lib.event.test.v0.models.TestEventUndefinedType(other))
              case None => sys.error("Union[TestEvent] requires a discriminator named 'discriminator' - this field was not found in the Json Value")
            }
          }
        }
      }
